...
export default function SymptomGuidance() {
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      role: "assistant",
      content:
        "Hello! I'm your AI health assistant. I can help you understand your symptoms and provide precautionary guidance. Please note that I don't provide medical diagnoses. How can I help you today?",
    },
  ]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Call model main backend (FastAPI on port 8001) for symptom → disease/advice
      const res = await fetch("/model-api/get_response", {
        method: "POST",
        headers: {
      ...
      if (!res.ok) {
        ...
        setMessages((prev) => [
          ...prev,
          {
            role: "assistant",
            content:
              "I'm having trouble responding at the moment. Please try again in a moment. This is not a medical diagnosis.",
          },
        ]);
        return;
      }

      // Model returns { disease, advice, confidence, interpreted } or { error }
      let reply: string;
      if (json?.disease && json?.advice) {
        reply = `Based on your symptoms, this may relate to: ${json.disease}. Advice: ${json.advice}${json.confidence ? ` (Confidence: ${json.confidence})` : ""}. This is not a medical diagnosis.`;
      } else if (json?.error) {
        reply = `${json.error} This is not a medical diagnosis.`;
      } else {
        reply = "I'm not sure how to interpret that. Could you describe your symptoms in a bit more detail? This is not a medical diagnosis.";
      }
      ...
      const msg =
        e instanceof Error
          ? e.message
          : "Unexpected error while contacting the symptom guidance service.";
      setError(msg);
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content:
            "I'm having trouble connecting right now. Please check your connection and try again. This is not a medical diagnosis.",
        },
      ]);
    } finally {
      setIsSending(false);
